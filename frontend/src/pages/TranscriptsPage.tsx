...
import {
  ...
  message, Popconfirm, Divider, AutoComplete, InputNumber, Switch, Tooltip, Empty, Select, Modal, Upload, Form,
} from 'antd';
import {
  UploadOutlined, FilePdfOutlined, CheckCircleOutlined, EditOutlined,
  DeleteOutlined, PlusOutlined, EyeOutlined, LockOutlined, SearchOutlined, DownloadOutlined,
  ArrowLeftOutlined, FileExcelOutlined, FileTextOutlined, BookOutlined, InfoCircleOutlined,
  HistoryOutlined,
...
import {
  getAllTranscripts, getTranscript, saveTranscriptGrades,
  searchTranscriptCourses, previewConversion, finalizeTranscript, revertTranscript,
  deleteTranscriptGradeEntry, getGradingSchemes, getEctsIkuConversion, getCoordinatorApplication,
  updateTranscriptGradeEntry, getUniversities, getUniversityCourses, deleteTranscript,
  createManualTranscript, getDepartments, getSchemeVersion, getSchemeVersions, updateTranscript
} from '../api/client';
...
const CONVERSION_METHOD_LABELS: Record<string, string> = {
...
interface GradeRow {
  ...
  ects_grade?: string;
  iku_grade?: string;
  conversion_mode: 'auto_local' | 'auto_ects' | 'manual';
...
export default function TranscriptsPage() {
  ...
  const isDetail = !!id;
  const isReadonlyRegistrar = (hasRole('registrar') || hasRole('faculty_affairs_admin'))
    && !user?.roles.some((r: any) => ['coordinator', 'dept_admin', 'super_admin'].includes(r.role));
  const isStudentOnly = hasRole('student')
    && !user?.roles.some((r: any) => ['coordinator', 'dept_admin', 'super_admin', 'registrar'].includes(r.role));
  ...
  const [editingIkuKey, setEditingIkuKey] = useState<string | null>(null);
  const [editIkuVal, setEditIkuVal] = useState('');
  const laPrefilled = useRef(false);

  // Filters for list view
  const [searchText, setSearchText] = useState('');
  const [statusFilter, setStatusFilter] = useState<string | null>(null);
  const [uniFilter, setUniFilter] = useState<number | string | undefined>(undefined);

  // Manual Import Modal State
  const [manualModalVisible, setManualModalVisible] = useState(false);
  const [manualUni, setManualUni] = useState<number | null>(null);
  const [manualUniName, setManualUniName] = useState('');
  const [manualStudentLabel, setManualStudentLabel] = useState('');
  const [manualStudentNumber, setManualStudentNumber] = useState('');
  const [manualFile, setManualFile] = useState<File | null>(null);
  const [manualLoading, setManualLoading] = useState(false);

  // Edit Transcript Modal State
  const [editingTranscript, setEditingTranscript] = useState<StudentTranscript | null>(null);
  ...
  // Find active department ID
  const activeDeptId = useMemo(() => {
    if (!activeDepartment) return null;
    const dept = allDepartments.find(d => d.code === activeDepartment);
    if (dept) return dept.id;
    if (!user) return null;
    // Try department_code, then faculty_code
    const role = user.roles.find(r => r.department_code === activeDepartment || r.faculty_code === activeDepartment);
    return role?.department_id || null;
  }, [user, activeDepartment, allDepartments]);

  // List query
  const { data: pendingList = [], isLoading: listLoading } = useQuery({
    queryKey: ['all-transcripts', activeDeptId],
    queryFn: () => getAllTranscripts(activeDeptId),
    enabled: !isDetail,
  });
  ...
  // Fetch linked application to pre-fill courses from LA
  const { data: linkedApplication } = useQuery({
    queryKey: ['transcript-application', transcript?.application_id],
    queryFn: () => transcript?.application_id ? getCoordinatorApplication(transcript.application_id) : Promise.resolve(null),
    enabled: !!transcript?.application_id,
  });

  const partnerUniId = transcript?.partner_university_id;
  ...
  const usedHomeCourseIds = useMemo(() => {
    const ids = new Set<number>();
    gradeRows.forEach(row => {
      if (row.key !== selectedRowKey) {
        row.mapped_home_course_ids?.forEach(id => ids.add(id));
      }
    });
    return ids;
  }, [gradeRows, selectedRowKey]);

  // ── Memoized University Options for Filters ──
  const uniFilterOptions = useMemo(() => {
    const optionsMap = new Map<string | number, { value: string | number, label: string }>();
    universities?.forEach((u: University) => {
      if (!u.is_home) {
        optionsMap.set(u.id, { value: u.id, label: u.name });
      }
    });
    (pendingList || []).forEach((t: any) => {
      const uniId = t.partner_university_id;
      const uniName = t.partner_university_name;
      if (uniId) {
        if (!optionsMap.has(uniId)) {
          optionsMap.set(uniId, { value: uniId, label: uniName || `Uni #${uniId}` });
        }
      } else if (uniName) {
        const alreadyExists = Array.from(optionsMap.values()).some(
          opt => opt.label.toLowerCase() === uniName.toLowerCase()
        );
        if (!alreadyExists && !optionsMap.has(uniName)) {
          optionsMap.set(uniName, { value: uniName, label: `${uniName} (Unregistered)` });
        }
      }
    });
    return Array.from(optionsMap.values()).sort((a, b) => a.label.localeCompare(b.label));
  }, [universities, pendingList]);

  // ── Memoized University Options for Manual Import ──
  const manualUniOptions = useMemo(() => {
    const optionsMap = new Map<string | number, { value: string, id?: number }>();
    universities?.forEach((u: University) => {
      if (!u.is_home) {
        optionsMap.set(u.id, { value: u.name, id: u.id });
      }
    });
    (pendingList || []).forEach((t: any) => {
      const name = t.partner_university_name;
      if (name && !t.partner_university_id) {
        const alreadyRegistered = (universities || []).some((u: University) =>
          u.name.toLowerCase() === name.toLowerCase()
        );
        if (!alreadyRegistered) {
          optionsMap.set(`custom-${name}`, { value: name });
        }
      }
    });
  ...
  const initGradeRows = useCallback((entries: TranscriptGradeEntry[]) => {
    setGradeRows(entries.map(e => {
      ...
      // Determine final mapped IDs:
      // 1. If there's a coordinator override in the LA, ALWAYS use it (it reflects the latest manual decision)
      // 2. Otherwise, if the entry already has mapped IDs in the DB, use those
      // 3. Otherwise, fall back to student's selection in LA
      ...
      return {
        ...
        conversion_mode: e.conversion_method === 'manual_override' ? 'manual' :
                         e.conversion_method === 'auto_ects' ? 'auto_ects' : 'auto_local',
  ...
  // Reset state when switching between transcripts
  useEffect(() => {
    setGradeRows([]);
  ...
  // Set PDF preview URL when transcript loads
  useEffect(() => {
  ...
  // Initialize grade rows when transcript data loads
  useEffect(() => {
  ...
  // Pre-fill from LA application — only once, when no grade entries exist yet
  useEffect(() => {
  ...
  const handlePreviewConversion = async (key: string) => {
    if (!id) return;
    const row = gradeRows.find(r => r.key === key);
    if (!row || row.conversion_mode === 'manual') return;
  ...
  const handleSaveGrades = async (): Promise<boolean> => {
    ...
    const hasEmptyGrades = gradeRows.some(r => {
      ...
      if (r.conversion_mode === 'manual') {
        return !r.iku_grade || r.iku_grade.trim() === '';
      }
      if (r.conversion_mode === 'auto_ects') {
        return !r.ects_grade || r.ects_grade.trim() === '';
      }
    ...
    // Manual mode: no conversion validation needed
    const hasInvalidConversions = gradeRows.some(r => {
      if (r.conversion_mode === 'manual') return false;
    ...
    setSaving(true);
    try {
      // Save new entries and capture returned data with IDs
      ...
      if (newRows.length > 0) {
        savedEntries = await saveTranscriptGrades(Number(id), newRows.map(r => ({
          ...
          iku_grade: r.conversion_mode === 'manual' ? r.iku_grade : undefined,
      ...
      await Promise.all(existingRows.map(row => {
        ...
        if (row.conversion_mode === 'manual') {
          updates.local_grade = row.local_grade || null;
          updates.ects_grade = row.ects_grade || null;
          updates.iku_grade = row.iku_grade || null;
          updates.conversion_method = 'manual_override';
        } else {
      ...
      // Update rows in-place to preserve user's ordering
      const savedByName = new Map<string, number>();
      savedEntries.forEach(e => {
        const name = (e.partner_course_name || '').toLowerCase().trim();
        if (name) savedByName.set(name, e.id);
      });
      setGradeRows(prev => prev.map(r => {
        if (r.existing_id) return r;
        const name = r.partner_course_name.toLowerCase().trim();
        const newId = savedByName.get(name);
        if (newId) return { ...r, key: `existing-${newId}`, existing_id: newId };
        return r;
      }));
  ...
  const handleDeleteEntry = async (entryId: number) => {
    ...
    try {
      ...
      // Refetch and re-initialize to ensure the UI is in sync
      const fresh = await getTranscript(Number(id));
      qc.setQueryData(['transcript', id], fresh);
      
      // We set rows to empty first to ensure any lingering state is cleared, 
      // then re-init from the fresh server data.
      setGradeRows([]);
      setTimeout(() => {
        initGradeRows(fresh.grade_entries || []);
      }, 0);
  ...
  const handleFinalize = async () => {
    if (!id) return;
    const saved = await handleSaveGrades();
    if (!saved) return;

    const hasErrors = gradeRows.some(r => r.conversion_mode !== 'manual' && (!r.iku_grade || r.iku_grade === '?' || r.iku_grade === '-'));
    if (hasErrors) {
      message.error(t('transcripts.messages.cannotFinalize'));
      return;
    }
    try {
      await finalizeTranscript(Number(id));
      message.success(t('transcripts.messages.transcriptFinalized'));
      qc.invalidateQueries({ queryKey: ['transcript', id] });
      qc.invalidateQueries({ queryKey: ['all-transcripts'] });
      qc.invalidateQueries({ queryKey: ['coordinator-dashboard'] });
      qc.invalidateQueries({ queryKey: ['coordinator-applications'] });
      qc.invalidateQueries({ queryKey: ['student-applications'] });
      qc.invalidateQueries({ queryKey: ['my-transcripts'] });
    } catch (e: any) {
      message.error(e.response?.data?.detail || t('transcripts.messages.finalizeFailed'));
    }
  };
  ...
  const handleRevert = async () => {
    if (!id) return;
    try {
      await revertTranscript(Number(id));
      message.success(t('transcripts.messages.transcriptReverted'));
      qc.invalidateQueries({ queryKey: ['transcript', id] });
      qc.invalidateQueries({ queryKey: ['all-transcripts'] });
      qc.invalidateQueries({ queryKey: ['coordinator-dashboard'] });
      qc.invalidateQueries({ queryKey: ['coordinator-applications'] });
      ...
      message.error(e.response?.data?.detail || t('transcripts.messages.revertFailed'));
    }
  };

  const handleManualImport = async () => {
    if (!manualUniName.trim()) {
      message.error(t('transcripts.messages.selectUniversity'));
      return;
    }
    setManualLoading(true);
    try {
      const name = manualStudentLabel.trim() || 'Anonymous Student';
      const fullLabel = manualStudentNumber.trim() 
        ? `${name} | ID: ${manualStudentNumber.trim()}`
        : name;

      const res = await createManualTranscript({
        university_id: manualUni,
        university_name: manualUniName,
        student_label: fullLabel,
        student_id: null,
        department_id: activeDeptId,
        file: manualFile,
      });
      message.success(t('transcripts.messages.historicalCreated'));
      ...
      setManualUni(null);
      setManualUniName('');
      await qc.invalidateQueries({ queryKey: ['all-transcripts'] });
      navigate(`/transcripts/${res.id}`);
    } catch (e: any) {
      message.error(e.response?.data?.detail || t('transcripts.messages.createFailed'));
    } finally {
      setManualLoading(false);
    }
  };

  const updateTranscriptMut = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => updateTranscript(id, data),
    onSuccess: () => {
      message.success(t('transcripts.messages.saved'));
      ...
      qc.invalidateQueries({ queryKey: ['all-transcripts', activeDeptId] });
      if (id) qc.invalidateQueries({ queryKey: ['transcript', id] });
    },
    onError: () => {
      message.error(t('transcripts.messages.saveFailed'));
    },
  });

  const renderEditModal = () => (
    <Modal
      title={t('transcripts.actions.editTranscript')}
      open={editModalVisible}
      onCancel={() => { setEditModalVisible(false); setEditingTranscript(null); setEditFile(null); setRemoveExistingFile(false); }}
      onOk={() => {
        editForm.validateFields().then(vals => {
          if (!editingTranscript) return;
          const payload: any = {
            student_name: (!vals.student_name || vals.student_name.trim() === '') ? '__EMPTY__' : vals.student_name,
            student_eid: (!vals.student_eid || vals.student_eid.trim() === '') ? '__EMPTY__' : vals.student_eid,
            file: editFile,
            remove_file: removeExistingFile,
          };
          updateTranscriptMut.mutate({ id: editingTranscript.id, data: payload });
        });
      }}
      confirmLoading={updateTranscriptMut.isPending}
    >
      <Form form={editForm} layout="vertical">
        <Form.Item name="student_name" label={t('transcripts.import.studentName')}>
          <Input />
        </Form.Item>
        <Form.Item name="student_eid" label={t('transcripts.import.studentNumber')}>
          <Input />
        </Form.Item>
        <Form.Item label={t('transcripts.import.transcriptPdf')}>
          <div style={{ marginBottom: 12 }}>
            {(!removeExistingFile && editingTranscript?.original_filename && !editFile) ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: '#f5f5f5', border: '1px solid #d9d9d9', borderRadius: 6, marginBottom: 8 }}>
                <Space>
                  <FilePdfOutlined style={{ color: '#ff4d4f' }} />
                  <Text strong>{editingTranscript.original_filename}</Text>
                </Space>
                <Button type="text" danger icon={<DeleteOutlined />} size="small" onClick={() => setRemoveExistingFile(true)}>
                  {t('transcripts.actions.delete')}
                </Button>
              </div>
            ) : removeExistingFile && !editFile ? (
              <Text type="danger" style={{ display: 'block', marginBottom: 8 }}>{t('transcripts.messages.fileWillBeRemoved', 'Mevcut dosya kaldırılacak')}</Text>
            ) : null}
            <Upload
              beforeUpload={(file) => {
              ...
              fileList={editFile ? [editFile as any] : []}
              maxCount={1}
              accept=".pdf"
            >
              <Button icon={<UploadOutlined />}>{t('transcripts.import.selectPdf')}</Button>
            </Upload>
          </div>
        </Form.Item>
      </Form>
  ...
  const isFinalized = transcript?.status === 'finalized';

  // ── Pending list columns ──
  const listColumns = [
    {
      title: <Text style={{ fontSize: 11, color: '#888' }}>{t('transcripts.columns.student')}</Text>,
      key: 'student',
      render: (_: any, rec: StudentTranscript) => {
        const isImport = !rec.student_id;
        if (isImport) {
          const rawLabel = rec.notes
            ?.replace('Manually created historical record: ', '')
            ?.replace('Manuel olarak oluşturulmuş geçmiş kayıt: ', '') || '';
          let [rawName, idPart] = rawLabel.split(' | ID: ');
          if (rawName && rawName.endsWith(' | ID:')) rawName = rawName.replace(' | ID:', '').trim();
          let name = rec.student_name || rawName;
          if (name && name.endsWith(' | ID:')) name = name.replace(' | ID:', '').trim();
          if (name === 'Anonymous Student' || name === 'Anonim Öğrenci') {
            name = t('transcripts.import.anonymousStudent');
          }

          return (
            <div>
              <div style={{ fontWeight: 500, color: '#262626' }}>{name || t('transcripts.detail.unknownStudent')}</div>
              <div style={{ fontSize: 12, color: '#8c8c8c' }}>{rec.student_eid || idPart || t('transcripts.detail.historicalRecord')}</div>
            </div>
          );
        }
        return (
          <div>
            <div style={{ fontWeight: 500 }}>{rec.student_name || `Student #${rec.student_id}`}</div>
            <div style={{ fontSize: 12, color: '#888' }}>{rec.student_eid}</div>
          </div>
        );
      },
    },
    {
      title: <Text style={{ fontSize: 11, color: '#888' }}>{t('transcripts.columns.partnerUniversity')}</Text>,
      key: 'uni',
      render: (_: any, rec: StudentTranscript) => rec.partner_university_name || '-',
    },
    {
      title: <Text style={{ fontSize: 11, color: '#888' }}>{t('transcripts.columns.fileSource')}</Text>,
      key: 'file',
      render: (_: any, rec: StudentTranscript) => {
        if (!rec.student_id && (!rec.original_filename || rec.original_filename.startsWith('Historical Import'))) {
          return <Tag color="geekblue">{t('transcripts.historicalImport')}</Tag>;
        }
        return <Text type="secondary" style={{ fontSize: 12 }}>{rec.original_filename || '—'}</Text>;
      },
    },
    {
      title: <Text style={{ fontSize: 11, color: '#888' }}>{t('transcripts.columns.department')}</Text>,
      dataIndex: 'department_code',
      key: 'dept',
      render: (code: string) => code ? <Tag color="orange">{code}</Tag> : '-',
    },
    {
      title: <Text style={{ fontSize: 11, color: '#888' }}>{t('transcripts.columns.status')}</Text>,
      dataIndex: 'status',
      key: 'status',
      render: (s: string) => {
    ...
    {
      title: <Text style={{ fontSize: 11, color: '#888' }}>{t('transcripts.columns.date')}</Text>,
    ...
    },
    {
      ...
      render: (_: any, rec: StudentTranscript) => (
        <Space size="small">
          <Button 
            type="primary" 
            onClick={(e) => { e.stopPropagation(); navigate(`/transcripts/${rec.id}`); }}
            style={{ background: '#c92a2a', borderColor: '#c92a2a', fontWeight: 500 }}
          >
            {t('transcripts.actions.review')}
          </Button>
          {rec.status === 'grading_in_progress' && !isReadonlyRegistrar && (
            <Tooltip title={t('transcripts.actions.edit')}>
              <Button 
                icon={<EditOutlined />} 
                onClick={(e) => {
                  e.stopPropagation();
                  setEditingTranscript(rec);
                  const isImport = !rec.student_id;
                  let studentName = rec.student_name;
                  if (isImport) {
                    const rawLabel = rec.notes
                      ?.replace('Manually created historical record: ', '')
                      ?.replace('Manuel olarak oluşturulmuş geçmiş kayıt: ', '') || '';
                    const [rawName] = rawLabel.split(' | ID: ');
                    studentName = rawName;
                  }
                  editForm.setFieldsValue({
                    student_name: studentName,
                    student_eid: rec.student_eid,
                  });
                  setRemoveExistingFile(false);
          ...
          {(rec.status === 'uploaded' || rec.status === 'grading_in_progress') && !isReadonlyRegistrar && (
            <Popconfirm
              title={t('transcripts.actions.deleteTranscript')}
              description={t('transcripts.actions.deleteTranscriptDesc')}
              onConfirm={async (e) => {
                e?.stopPropagation();
                try {
                  await deleteTranscript(rec.id);
                  message.success(t('transcripts.messages.transcriptDeleted'));
                  qc.invalidateQueries({ queryKey: ['all-transcripts'] });
                } catch (err: any) {
                  message.error(t('transcripts.messages.deleteFailed'));
                }
              }}
              onCancel={(e) => e?.stopPropagation()}
              okText={t('transcripts.actions.delete')}
              cancelText={t('transcripts.actions.cancel')}
  ...
  const gradeColumns = [
    {
      ...
      render: (_: any, row: GradeRow) => (
        <AutoComplete
          ...
          placeholder="Search..."
          filterOption={false}
          disabled={isFinalized || isReadonlyRegistrar}
        />
      ),
    },
    {
      ...
      render: (val: string | null, row: GradeRow) => (
        <Input
          value={val || ''}
          onChange={e => updateRow(row.key, 'partner_course_code', e.target.value)}
          size="small"
    ...
    },
    {
      ...
      render: (val: number | null, row: GradeRow) => (
        <InputNumber
          value={val}
          onChange={v => updateRow(row.key, 'partner_ects', v)}
          size="small"
          min={0}
          max={30}
          step={0.5}
    ...
    },
    {
      ...
      render: (_: any, row: GradeRow) => (
        <Select
          ...
          onChange={(v: any) => {
            const newMode = v as 'auto_local' | 'auto_ects' | 'manual';
          ...
          options={isStudentOnly
            ...
            : [
                { label: t('transcripts.gradeTable.modes.localToIku'), value: 'auto_local' },
                { label: t('transcripts.gradeTable.modes.ectsToIku'), value: 'auto_ects' },
                { label: t('transcripts.gradeTable.modes.manual'), value: 'manual' },
              ]
          }
        />
      ),
    },
    {
      ...
      render: (val: string, row: GradeRow) => (
        <Input
          value={val}
          onChange={e => updateRow(row.key, 'local_grade', e.target.value)}
    ...
    },
    {
      ...
      render: (val: string | undefined, row: GradeRow) => (
        <Input
          value={val || ''}
          onChange={e => updateRow(row.key, 'ects_grade', e.target.value)}
          ...
          size="small"
          style={{ width: '100%' }}
          disabled={isFinalized || row.conversion_mode === 'auto_local'}
        />
      ),
    },
    {
      ...
      align: 'center' as const,
      render: (val: string | undefined, row: GradeRow) => {
        // Manual mode: direct editable input (only for coordinator/admin)
        if (row.conversion_mode === 'manual' && !isStudentOnly) {
          return (
            <Input
              value={val || ''}
              onChange={e => updateRow(row.key, 'iku_grade', e.target.value.toUpperCase())}
              placeholder=""
              size="small"
        ...
        // Auto mode: read-only with info tooltip
        return (
          <Space size={2}>
            ...
            {row.conversion_method && (
              <Tooltip title={
                <div>
                  {row.conversion_method === 'manual_override' ? (
                    <div><strong>{t('transcripts.gradeTable.conversionMethod')}:</strong> {t('transcripts.gradeTable.manualOverride')}</div>
                  ) : (
                    <>
                      <div><strong>{t('transcripts.gradeTable.conversionMethod')}:</strong> {row.conversion_method === 'auto_ects' ? 'ECTS → IKU' : row.conversion_method === 'auto_local' ? t('transcripts.gradeTable.localToEcts') : row.conversion_method}</div>
    ...
    },
    {
      ...
      render: (_: any, row: GradeRow) => (
        <Space size={2}>
          {!isFinalized && !isReadonlyRegistrar && (
            <Button
              type="link"
              danger
              size="small"
  ...
  // ── Render: Pending List ──
  if (!isDetail) {
    const filteredList = pendingList.filter((t: StudentTranscript) => {
      const isHistorical = !t.student_id;
      const displayText = isHistorical
        ? (t.partner_university_name || '')
        : (t.student_name || t.student_eid || '');
      const matchSearch = displayText.toLowerCase().includes(searchText.toLowerCase()) ||
                          (t.partner_university_name || '').toLowerCase().includes(searchText.toLowerCase());
      const matchStatus = statusFilter ? t.status === statusFilter : true;
      const matchUni = uniFilter 
        ? (typeof uniFilter === 'number' ? t.partner_university_id === uniFilter : t.partner_university_name === uniFilter)
        : true;
    ...
    return (
      <div style={{ padding: '24px', maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
          <div>
            <Title level={2} style={{ margin: 0 }}>{t('transcripts.coordinatorPanel')}</Title>
            <Text type="secondary" style={{ fontSize: 16 }}>
              {t('transcripts.coordinatorPanelDesc')}
            </Text>
          </div>
          {!isReadonlyRegistrar && (
            <Button
              ...
              style={{ background: '#c92a2a', borderColor: '#c92a2a', height: 40, borderRadius: 8 }}
            >
              {t('transcripts.importHistorical')}
            </Button>
          )}
        </div>

        <Card
          style={{ marginBottom: 24, borderRadius: 12, border: 'none', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}
          bodyStyle={{ padding: '16px 24px' }}
        >
          <Space size="middle" wrap>
            <Select
              placeholder={t('transcripts.filters.byStatus')}
              allowClear
              style={{ width: 180 }}
              value={statusFilter}
              onChange={setStatusFilter}
              options={Object.entries(TRANSCRIPT_STATUS_CONFIG).map(([key, cfg]) => ({ value: key, label: t(cfg.label) }))}
            />
            <Select
              style={{ width: 250 }}
              allowClear
              showSearch
              placeholder={t('transcripts.filters.byUniversity')}
              value={uniFilter}
            ...
            />
            <Input
              placeholder={t('transcripts.filters.searchStudent')}
        ...
        <div style={{
          ...
          <Table
            ...
            locale={{ emptyText: <Empty description={t('transcripts.noTranscripts')} /> }}
          />
        </div>

        <Modal
          title={t('transcripts.importModalTitle')}
          ...
          okText={t('transcripts.createRecord')}
          okButtonProps={{ style: { background: '#c92a2a', borderColor: '#c92a2a' } }}
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, paddingTop: 16 }}>
            <div>
              <Text strong>{t('transcripts.import.partnerUniversity')}</Text>
              <AutoComplete
                style={{ width: '100%', marginTop: 8 }}
                placeholder={t('transcripts.import.searchOrType')}
                value={manualUniName}
                options={manualUniOptions}
                onSelect={(val: string, option: any) => {
                  setManualUni(option.id || null);
                  setManualUniName(val);
                }}
                onChange={(val) => {
                  setManualUniName(val);
                  const exists = universities.find((u: University) => u.name === val);
                  if (exists) {
                    setManualUni(exists.id);
                  } else {
                    setManualUni(null);
                  }
                }}
                filterOption={(inputValue, option) =>
                  !!option?.value && String(option.value).toUpperCase().includes(inputValue.toUpperCase())
                }
              />
              <Text type="secondary" style={{ fontSize: 12 }}>
                {t('transcripts.import.selectOrType')}
              </Text>
            </div>
            <div style={{ display: 'flex', gap: 12 }}>
              <div style={{ flex: 2 }}>
                <Text strong>{t('transcripts.import.studentName')}</Text>
                <Input 
                  style={{ marginTop: 8 }}
                  placeholder={t('transcripts.import.studentNamePlaceholder')} 
                  value={manualStudentLabel}
                  onChange={e => setManualStudentLabel(e.target.value)}
                />
              </div>
              <div style={{ flex: 1 }}>
                <Text strong>{t('transcripts.import.studentNumber')}</Text>
                <Input 
                  style={{ marginTop: 8 }}
                  placeholder={t('transcripts.import.studentNumberPlaceholder')} 
            ...
            <Text type="secondary" style={{ fontSize: 12 }}>
              {t('transcripts.import.historicalHint')}
            </Text>
            <div>
              <Text strong>{t('transcripts.import.transcriptPdf')}</Text>
              <div style={{ marginTop: 8 }}>
                <Upload
                  ...
                  fileList={manualFile ? [manualFile as any] : []}
                  maxCount={1}
                  accept=".pdf"
                >
                  <Button icon={<UploadOutlined />}>{t('transcripts.import.selectPdf')}</Button>
              ...
              <Text type="secondary" style={{ fontSize: 12 }}>
                {t('transcripts.import.uploadHint')}
              </Text>
            </div>
          </div>
        </Modal>
  ...
  const handleExportPDF = () => {
    ...
    const homeUni = homeUniversity?.name || 'İstanbul Kültür Üniversitesi';
    const headers = [
  ...
  // Ensure transcript is not null for the detail render below
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Button onClick={() => navigate('/transcripts')}>{t('transcripts.actions.back')}</Button>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Button icon={<HistoryOutlined />} onClick={() => setAuditModalOpen(true)}>
            {t('transcripts.detail.auditReport', 'Audit Report')}
          </Button>
          {isFinalized && (
            <>
              <Button
              ...
              >
                {t('transcripts.detail.exportDocx')}
              </Button>
              <Button
            ...
            <>
              <Button
                ...
                onClick={() => {
                  ...
                  const isImport = !transcript.student_id;
                  let studentName = transcript.student_name;
                  if (isImport) {
                    const rawLabel = transcript.notes
                      ?.replace('Manually created historical record: ', '')
                      ?.replace('Manuel olarak oluşturulmuş geçmiş kayıt: ', '') || '';
                    const [rawName] = rawLabel.split(' | ID: ');
                    studentName = rawName;
                  ...
                  editForm.setFieldsValue({
                    student_name: studentName,
                    student_eid: transcript.student_eid,
                  });
                  setRemoveExistingFile(false);
                  setEditModalVisible(true);
                }}
              >
                {t('transcripts.actions.edit')}
              </Button>
              <Popconfirm
                title={t('transcripts.actions.finalizeConfirm')}
                description={t('transcripts.actions.finalizeDesc')}
                onConfirm={handleFinalize}
                disabled={gradeRows.some(r => !r.iku_grade || r.iku_grade === '?' || r.iku_grade === '-')}
              >
                <Button
                  type="primary"
                  icon={<LockOutlined />}
                  disabled={gradeRows.some(r => !r.iku_grade || r.iku_grade === '?' || r.iku_grade === '-')}
                  title={gradeRows.some(r => !r.iku_grade || r.iku_grade === '?' || r.iku_grade === '-') ? t('transcripts.actions.finalizeFixGrades') : ""}
                >
                  {t('transcripts.actions.finalize')}
                </Button>
              </Popconfirm>
            </>
          )}
        ...
        <Card size="small" style={{ flex: 1 }}>
          ...
          {(() => {
            ...
            if (isImport) {
              ...
              let [rawName, idPart] = rawLabel.split(' | ID: ');
              if (rawName && rawName.endsWith(' | ID:')) rawName = rawName.replace(' | ID:', '').trim();
              let name = transcript.student_name || rawName;
              if (name && name.endsWith(' | ID:')) name = name.replace(' | ID:', '').trim();
              if (name === 'Anonymous Student' || name === 'Anonim Öğrenci') {
                name = t('transcripts.import.anonymousStudent');
              }
              return (
                <div>
                  <div style={{ fontWeight: 500 }}>{name || t('transcripts.detail.unknownStudent')}</div>
                  <div style={{ fontSize: 12, color: '#8c8c8c' }}>{transcript.student_eid || idPart || t('transcripts.detail.historicalRecord')}</div>
                </div>
              );
            }
            return (
            ...
            {schemeInfo && (
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    // Use the grading scheme version actually referenced by the entries or transcript,
                    // NOT the latest version. Entries preserve their version on creation.
                    ...
                    if (!detail) {
                      // Fallback: get latest version only if no entries exist yet
            ...
            footer={() => (
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                {!isFinalized && !isReadonlyRegistrar ? (
                  ...
                  <Popconfirm
                    title={t('transcripts.actions.revertConfirm')}
                    description={t('transcripts.actions.revertDesc')}
                    onConfirm={handleRevert}
                    okText={t('transcripts.actions.yes')}
                    cancelText={t('transcripts.actions.no')}
                  >
                    <Button type="default">
                      {t('transcripts.actions.revert')}
                    </Button>
                  </Popconfirm>
                ) : <div />}
              </div>
            ...
            >
              <div style={{ marginBottom: 12 }}>
                <Text type="secondary" style={{ fontSize: 13 }}>
                  {t('transcripts.matching.description')}
                </Text>
              </div>
              <Select
                mode="multiple"
          ...
          columns={[
            {
              ...
              render: (_row: any, record: GradeRow) => (
                <div style={{ padding: '8px 0' }}>
                  <Text strong>{record.partner_course_code || '—'}</Text>
                  <br />
                  <Text>{record.partner_course_name}</Text>
                  <br />
            ...
            },
            {
              ...
              key: 'home',
              render: (_row: any, record: GradeRow) => {
                // Find matching selection from linked application
                const matchingSelection = linkedApplication?.selections?.find((s: any) => {
                  if (s.status === 'not_selected' || s.status === 'rejected') return false;
                  if (record.partner_course_id && s.partner_course?.id === record.partner_course_id) return true;
                  if (record.partner_course_code && s.partner_course?.course_code === record.partner_course_code) return true;
                      ...
                      {homeCourses.map((hc: any) => (
                        <div key={hc.id} style={{ padding: '8px', background: '#f5f5f5', borderRadius: 6 }}>
                          ...
                          <Space style={{ marginTop: 4 }}>
                            ...
                            {isOverridden && (
                              <Tag color="orange" style={{ margin: 0 }}>{t('transcripts.overview.override')}</Tag>
                            )}
                          </Space>
                        </div>
                      ))}
                ...
                // Fallback for direct transfer or missing LA data
                return (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div style={{ padding: '8px', background: '#f5f5f5', borderRadius: 6 }}>
                      <Text strong>{t('transcripts.overview.directTransfer')}</Text>
                      <br />
                      <Space style={{ marginTop: 4 }}>
      ...
      {/* Audit Report Modal */}
      <Modal
        title={t('transcripts.detail.auditReport', 'Audit Report')}
        open={auditModalOpen}
        onCancel={() => setAuditModalOpen(false)}
        footer={null}
        width={700}
      >
        {transcript?.grade_entries?.filter((e: any) => (e.audit_log || []).length > 0).map((e: any) => (
          <Card key={e.id} size="small" style={{ marginBottom: 8 }} title={`${e.partner_course_name} (${e.local_grade || e.ects_grade} → ${e.iku_grade})`}>
            <div style={{ fontSize: 11, color: '#888', marginBottom: 6 }}>
              {e.grading_scheme_name && <span>{t('transcripts.detail.scheme')}: <Tag>{e.grading_scheme_name}</Tag></span>}
              {e.grading_scheme_version_number && <span> v{e.grading_scheme_version_number}</span>}
              {e.ects_iku_version_number && <span> | ECTS-IKU v{e.ects_iku_version_number}</span>}
            </div>
            {e.audit_log.map((a: any) => (
              <div key={a.id} style={{ display: 'flex', gap: 12, padding: '4px 0', borderBottom: '1px solid #f0f0f0' }}>
                <Tag color={a.is_manual_override ? 'orange' : 'blue'}>{a.conversion_method}</Tag>
                <span>{a.source_grade} → {a.target_iku_grade}</span>
                {a.previous_iku_grade && <Tag>was {a.previous_iku_grade}</Tag>}
                {a.is_manual_override && <Tag color="red">Manual</Tag>}
                <Text type="secondary" style={{ fontSize: 12 }}>{a.created_at ? new Date(a.created_at).toLocaleString() : ''}</Text>
              </div>
            ))}
          </Card>
        ))}
        {!transcript?.grade_entries?.some((e: any) => (e.audit_log || []).length > 0) && (
          <Empty description="No audit records" />
        )}
      </Modal>
